...
import numpy as np
import matplotlib
# Se no macOS der erro de backend, descomente abaixo:
...
class IndicePerturbacaoMIDI:
    ...
    def __init__(self):
        ...
        self.figura_unidade_tempo = "Semínima"  # Para referência
    ...
    def plotar_grafico(self, max_labels=20, mostrar_apenas_principais=True):
        ...
        nome_sem_ext = self.nome_arquivo.rsplit(".", 1)[0] if self.nome_arquivo else "MIDI"
        ax.set_title(f"Índices de perturbação ({nome_sem_ext})", fontsize=14, fontweight="bold")
        ax.grid(True, alpha=0.3)
        # Caixa de texto removida (não terá legenda)
        # ax.legend(loc="upper right")
        plt.tight_layout()
        return fig, ax


def analisar_arquivo_midi(caminho_arquivo: str, max_eventos: int | None = None, eventos_inicio: int = 0):
    print("\n" + "=" * 70)
    print("🎵 ANÁLISE DE ÍNDICES DE PERTURBAÇÃO MIDI - TEMPO EM BEATS")
    print("=" * 70)
    analisador = IndicePerturbacaoMIDI()
    eventos = analisador.carregar_midi(
        caminho_arquivo, max_eventos=max_eventos, eventos_inicio=eventos_inicio
    )
    if eventos:
        analisador.calcular_indices_perturbacao()
        analisador.gerar_relatorio_completo()
        fig, _ = analisador.plotar_grafico(
            max_labels=20, mostrar_apenas_principais=True
        )
        if fig:
            plt.show()
        return analisador
...
if __name__ == "__main__":
    arquivo_midi = "base.midi"  # Coloque o nome do seu arquivo aqui
    max_eventos = None  # None = todos os eventos; ou coloque um número
    eventos_inicio = 0  # Índice inicial

    if os.path.exists(arquivo_midi):
        analisar_arquivo_midi(
            arquivo_midi, max_eventos=max_eventos, eventos_inicio=eventos_inicio
        )
        print("\n✅ Análise concluída!")
        print("📊 O gráfico deve estar em uma janela separada")
    else:
        print(f"\n❌ Arquivo '{arquivo_midi}' não encontrado!")
        print(f"📍 Verifique se está na pasta: {os.getcwd()}")
        import glob
        ...
        if midis:
            ...
            for m in midis:
                print(f"   • {m}")
            print(f"\nTente usar: arquivo_midi = '{midis[0]}'")
        print("\n💡 Dica: coloque o arquivo .mid na mesma pasta deste script")
